...
export default function HomeCommand(){
 ...
 const send=async()=>{const message=text.trim();if(!message||busy)return;setBusy(true);setStatus('');try{await submitMessage(message,agent);setText('');setStatus(`${person} lucrează la răspuns…`);setConversation(c=>[...c,{sender:'owner',kind:'request',body:message,at:new Date().toISOString(),agent}].slice(-10))}catch(e:any){setStatus(`Nu am putut trimite: ${e?.message||'eroare'}`)}finally{setBusy(false)}}
 const toggleMic=()=>{
 ...
 return <div className="home-conversation-shell">
   <div className="home-dialogue" aria-live="polite">
