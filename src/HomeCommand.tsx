...
export default function HomeCommand(){
 ...
 useEffect(()=>{if(!visible)return;let dead=false;const load=async()=>{try{const feed=await getBiancaConversationFeed(40);if(dead)return;const rows=feed.filter((x:any)=>x.sender==='owner'||x.sender==='agent'||x.kind==='request'||x.kind==='result').slice(-10);setConversation(rows)}catch{}};void load();const t=setInterval(load,2000);return()=>{dead=true;clearInterval(t)}},[visible])
 if(!visible)return null
 const agent=agentMap[person]||'personal_assistant'
 const send=async()=>{const message=text.trim();if(!message||busy)return;setBusy(true);setStatus('');setExpanded(true);try{await submitMessage(message,agent);setText('');setStatus(`${person} lucrează la răspuns…`);setConversation(c=>[...c,{sender:'owner',kind:'request',body:message,at:new Date().toISOString(),agent}].slice(-10))}catch(e:any){setStatus(`Nu am putut trimite: ${e?.message||'eroare'}`)}finally{setBusy(false)}}
