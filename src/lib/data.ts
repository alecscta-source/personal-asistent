...
let loginRequest:Promise<any>|null=null
export async function sendLoginLink(email:string){
 ...
 loginRequest=(async()=>{try{return await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:window.location.origin}})}catch(err:any){return{data:{user:null,session:null},error:err instanceof Error?err:new Error(String(err||'Eroare necunoscută la autentificare'))}}})()
...
export async function uploadBiancaFiles(files:File[],message='',agent='personal_assistant'){
 if(!supabase)throw new Error('Supabase nu este configurat');const session=(await supabase.auth.getSession()).data.session;if(!session)throw new Error('Autentificare necesară');if(!files.length)return[]
 const uploaded:any[]=[]
 try{
  for(const file of files){const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');const path=`${session.user.id}/${Date.now()}-${crypto.randomUUID()}-${safe}`;const{error:upErr}=await supabase.storage.from('bianca-files').upload(path,file,{contentType:file.type||undefined,upsert:false});if(upErr)throw upErr;uploaded.push({path,name:file.name,mime:file.type||'application/octet-stream',size:file.size})}
  const{data,error}=await supabase.rpc('register_bianca_upload_batch',{p_files:uploaded,p_message:message,p_agent:agent});if(error)throw error
  return uploaded.map(f=>({task_id:data,...f}))
 }catch(err){await Promise.allSettled(uploaded.map(f=>supabase!.storage.from('bianca-files').remove([f.path])));throw err}
}
